...
export default async function handler(request, response) {
  ...
  try {
    ...
    await resendEmail(apiKey, {
      ...
      to: [contactEmail],
      reply_to: email,
      subject: `New 1Forge Designs signup · ${template || source}`,
      html: `<div style="font-family:Inter,Arial,sans-serif;max-width:560px;margin:0 auto;padding:36px 24px;background:#0b0b0c;color:#f3efe8"><p style="color:#f06a2a;letter-spacing:.18em;font-size:11px">NEW DESIGNS SIGNUP</p><h1 style="font-family:Georgia,serif;font-weight:400">Someone entered the forge.</h1><table style="width:100%;border-collapse:collapse;margin-top:28px"><tr><td style="padding:14px 0;border-top:1px solid #333;color:#999">Email</td><td style="padding:14px 0;border-top:1px solid #333;text-align:right"><a style="color:#fff" href="mailto:${safeEmail}">${safeEmail}</a></td></tr><tr><td style="padding:14px 0;border-top:1px solid #333;color:#999">Source</td><td style="padding:14px 0;border-top:1px solid #333;text-align:right">${safeSource}</td></tr><tr><td style="padding:14px 0;border-block:1px solid #333;color:#999">Template</td><td style="padding:14px 0;border-block:1px solid #333;text-align:right">${safeTemplate}</td></tr></table></div>`,
    })
    ...
    if (process.env.SEND_SIGNUP_CONFIRMATION !== 'false') {
      ...
      await resendEmail(apiKey, {
        from,
        to: [email],
        subject: isFreeDrop ? `${template || 'Your free Friday drop'} · 1Forge Designs` : 'You’re on the 1Forge Designs launch list',
        html: `<div style="font-family:Inter,Arial,sans-serif;max-width:560px;margin:0 auto;padding:40px 24px;background:#0b0b0c;color:#f3efe8"><p style="color:#f06a2a;letter-spacing:.18em;font-size:11px">1FORGE DESIGNS</p><h1 style="font-family:Georgia,serif;font-size:42px;line-height:1;font-weight:400;margin:24px 0">Welcome to the forge.</h1><p style="color:#ccc;line-height:1.7">${isFreeDrop ? `You’re registered for <strong>${safeTemplate}</strong>, this week’s free Friday release.` : 'You’ll be among the first to receive launch files, founding pricing and genuinely useful design notes.'}</p>${action}<p style="margin-top:34px;color:#777;font-size:12px">If you didn’t request this, you can ignore this email.</p></div>`,
      })
    }

    return response.status(200).json({
      success: true,
      message: isFreeDrop ? 'You’re in. Check your inbox for this week’s drop.' : 'You’re on the list. Check your inbox for confirmation.',
    })
  } catch (error) {
